    ...
    return (
      <main className="min-h-screen bg-[#050511] flex items-center justify-center p-6 text-white font-sans">
        <div className="w-full max-w-md bg-[#0a0a1a] border border-gray-800 rounded-3xl p-8 text-center shadow-[0_0_40px_rgba(6,182,212,0.1)] relative overflow-hidden">
          ...
          <h2 className="text-3xl font-black mb-2 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-fuchsia-500">Admin Access</h2>
          <p className="text-gray-400 mb-8 text-sm">Enter the secret passcode to add games.</p>
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="password" value={passcode} onChange={(e) => setPasscode(e.target.value)} placeholder="Enter Passcode..." className="w-full bg-[#121220] border border-gray-700 text-center text-white px-4 py-3 rounded-xl focus:outline-none focus:border-cyan-500 transition-all tracking-widest" />
            {authError && <p className="text-red-500 text-sm font-bold">{authError}</p>}
            <button type="submit" className="w-full bg-gradient-to-r from-fuchsia-600 to-purple-700 hover:from-fuchsia-500 hover:to-purple-600 text-white font-bold py-3 rounded-xl transition-all shadow-lg">Unlock Dashboard</button>
          </form>
          <Link href="/" className="block mt-6 text-sm text-gray-500 hover:text-white transition-colors">← Back to Home</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#050511] text-white p-6 md:p-12 font-sans flex items-center justify-center">
      <div className="w-full max-w-2xl bg-[#0a0a1a] border border-gray-800 rounded-3xl p-8 shadow-[0_0_50px_rgba(192,38,211,0.1)] relative">
        <Link href="/" className="inline-flex items-center text-sm text-gray-400 hover:text-fuchsia-400 mb-6 font-semibold"><span className="mr-2">←</span> Back to Arcade</Link>
        <h1 className="text-4xl font-black mb-8 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-fuchsia-500">Add New Game</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button type="submit" disabled={loading} className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-4 rounded-xl mt-4 transition-all active:scale-95">
