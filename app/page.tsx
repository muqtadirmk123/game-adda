...
export default function Home() {
  ...
  const [systemMessage, setSystemMessage] = useState<string | null>(null); // For Game Not Found error
          ...
          if (cmd && viewStateRef.current === 'dashboard') {
             setSelectedIndex((prev) => {
               ...
               if (cmd === 'SELECT') {
                 ...
                 // If game exists AND has an ID, push. Else show error.
                 if (selectedGame && selectedGame.id) {
                   router.push(`/game/${selectedGame.id}`);
                 } else {
                   setSystemMessage("⚠️ GAME NOT AVAILABLE YET");
  ...
  const SystemToast = () => {
    ...
    return (
      <div className="absolute top-10 left-1/2 -translate-x-1/2 z-[100] bg-red-600 text-white px-8 py-4 rounded-full font-black text-xl tracking-widest shadow-[0_0_30px_rgba(220,38,38,0.8)] animate-bounce border-2 border-white">
        {systemMessage}
      </div>
    );
  };
  ...
  return (
    <main className="min-h-screen bg-[#111] text-white font-sans">
      ...
      <section className="max-w-7xl mx-auto px-6 py-32 flex flex-col items-center text-center">
        ...
        <button onClick={() => setViewState('pairing')} className="bg-[#1ed760] text-black px-12 py-5 rounded-full text-2xl font-bold hover:scale-105 active:scale-95 transition-transform">
          Start playing now
        </button>
      </section>
    </main>
