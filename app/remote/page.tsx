...
function RemoteController() {
  ...
  if (remoteState === 'lobby') {
    return (
      <main className="fixed inset-0 bg-[#1a1a1a] text-white flex flex-col items-center justify-between py-20 px-6 font-sans">
        <div className="flex flex-col items-center">
          <div className="w-32 h-32 bg-[#42a82a] rounded-full flex items-center justify-center text-6xl font-black text-white mb-4 shadow-[0_0_30px_rgba(66,168,42,0.4)]">A</div>
  ...
  // 📱 PORTRAIT CONTROLLER (AirConsole Screenshot Style)
  return (
    <main className="fixed inset-0 w-full h-[100dvh] bg-[#1c1c1c] text-white flex flex-col items-center justify-between py-10 px-4 touch-none select-none overflow-hidden font-sans">
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="w-24 h-24 bg-[#42a82a] rounded-full flex items-center justify-center text-5xl font-black text-white shadow-xl border-4 border-[#222]">A</div>
        <h2 className="mt-2 text-lg font-bold flex items-center gap-1"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25z"/></svg> AMK</h2>
      </div>

      {/* Action Row (Search, Grid, Fav, Mute) */}
      <div className="flex items-center justify-center gap-4 w-full px-4 border-y border-[#333] py-4 bg-[#111]/30">
        <button onTouchStart={() => sendCommand('SEARCH')} className="w-12 h-12 rounded-full bg-[#2a2a2a] flex items-center justify-center active:bg-white/20 transition-colors border border-[#444]">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
        </button>
        <button onTouchStart={() => sendCommand('HOME')} className="w-12 h-12 rounded-full bg-[#2a2a2a] flex items-center justify-center active:bg-white/20 transition-colors border border-[#444]">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
        </button>
        <button onTouchStart={() => sendCommand('FAV')} className="w-12 h-12 rounded-full bg-[#2a2a2a] flex items-center justify-center active:bg-white/20 transition-colors border border-[#444]">
      ...
      <div className="relative w-64 h-64 bg-[#2a2a2a] rounded-full shadow-[0_0_30px_rgba(0,0,0,0.8)] border-[6px] border-[#222] my-4">
        <button onTouchStart={() => sendCommand('UP')} className={`absolute top-0 left-1/4 w-1/2 h-[35%] rounded-t-full flex items-start justify-center pt-4 transition-all ${activeBtn === 'UP' ? 'bg-white/10' : ''}`}>
        ...
        <button onTouchStart={() => sendCommand('DOWN')} className={`absolute bottom-0 left-1/4 w-1/2 h-[35%] rounded-b-full flex items-end justify-center pb-4 transition-all ${activeBtn === 'DOWN' ? 'bg-white/10' : ''}`}>
        ...
        <button onTouchStart={() => sendCommand('LEFT')} className={`absolute left-0 top-1/4 w-[35%] h-1/2 rounded-l-full flex items-center justify-start pl-4 transition-all ${activeBtn === 'LEFT' ? 'bg-white/10' : ''}`}>
          <div className="w-0 h-0 border-y-[8px] border-r-[12px] border-y-transparent border-r-gray-400"></div>
        </button>
        <button onTouchStart={() => sendCommand('RIGHT')} className={`absolute right-0 top-1/4 w-[35%] h-1/2 rounded-r-full flex items-center justify-end pr-4 transition-all ${activeBtn === 'RIGHT' ? 'bg-white/10' : ''}`}>
          <div className="w-0 h-0 border-y-[8px] border-l-[12px] border-y-transparent border-l-gray-400"></div>
        </button>
